...
function digitPressed(digitId, tab) {
	...
	if(tab == "night") { buttonList = ["nb0","nb1","nb2","nb3","nb4","nb5"]; }
	var selAll = true;
...
function updateSettings() {
	...
	settings.NightModeOff = Number(document.getElementById("nightModeOff").checked);
	settings.Speed = Number(document.getElementById("speed").value);
	settings.SetNight = document.getElementById("NightModeTest").checked;
	settings.StartTime = Number(document.getElementById("nightStart").value.substring(0,2) * 60) + Number(document.getElementById("nightStart").value.substring(3,5));
	settings.EndTime = Number(document.getElementById("nightEnd").value.substring(0,2) * 60) + Number(document.getElementById("nightEnd").value.substring(3,5));
}
//Color update
...
function changeSampleColor(h,s,v,tab) {
	...
	style.innerHTML =`.` + tab + `Color{ border-radius: 15px; height: 150px; background:rgb(`+tempV.r+`,`+tempV.g+`,`+tempV.b+`);}`;
}
...
function changeSlideBackground(h,s,v,tab) {
	...
	style.innerHTML = `.h-`+ tab +`-slide {outline: 0; -webkit-appearance: none; height: 8px; width: 100%; background: linear-gradient(to right,hsl(0,100%,50%),hsl(60,100%,50%),hsl(120,100%,50%),hsl(180,100%,50%),hsl(240,100%,50%),hsl(300,100%,50%),hsl(360,100%,50%)); border-radius: 20px;}
	.s-`+ tab +`-slide { outline: 0; -webkit-appearance: none; height: 8px; width: 100%; background: linear-gradient(to right,hsl(`+ hslider.value+`,0%,100%),hsl(`+hslider.value+`,50%,50%),hsl(`+ hslider.value+`,100%,50%)); border-radius: 20px;}
	.v-`+ tab +`-slide { outline: 0; -webkit-appearance: none; height: 8px; width: 100%; background: linear-gradient(to right,rgb(`+temp0.r+`,`+temp0.g+`,`+temp0.b+`),rgb(`+temp1.r+`,`+temp1.g+`,`+temp1.b+`),rgb(`+temp2.r+`,`+temp2.g+`,`+temp2.b+`));border-radius: 20px;}`;
}
function changeSlider(h,s,v,tab){
	...
	vslider.value = v;
	style.innerHTML = `
		.h-`+ tab +`-slide::-webkit-slider-thumb { border: .5px solid #000000; height: 23px; width: 23px; border-radius: 50px; background: rgb(`+tempH.r+`,`+tempH.g+`,`+tempH.b+`);  cursor: pointer;  -webkit-appearance: none;}
		.s-`+ tab +`-slide::-webkit-slider-thumb { border: .5px solid #000000; height: 23px; width: 23px; border-radius: 50px; background: rgb(`+tempS.r+`,`+tempS.g+`,`+tempS.b+`); cursor: pointer; -webkit-appearance: none;}
		.v-`+ tab +`-slide::-webkit-slider-thumb { border: .5px solid #000000; height: 23px; width: 23px; border-radius: 50px; background: rgb(`+tempV.r+`,`+tempV.g+`,`+tempV.b+`); cursor:	pointer; -webkit-appearance: none;}`;
}
...
function LoadSettings() {
	...
	var lastColor = hsvToRgb(settings.H[0],settings.S[0]*100,settings.V[0]*100);
	var sameColor = true;
	buttonList.forEach(function(id, i){
		var temp = hsvToRgb(settings.H[i],settings.S[i]*100,settings.V[i]*100);
		if (lastColor.r != temp.r && lastColor.g != temp.g && lastColor.g != temp.g) {
			sameColor = false;
		}
		const button = document.getElementById(id);
		if(button.classList.contains("btn-secondary")){
	...
	lastColor = hsvToRgb(settings.NightH[0],settings.NightS[0]*100,settings.NightV[0]*100);
	sameColor = true;
	nightButtonList.forEach(function(id, i){
		var temp = hsvToRgb(settings.NightH[i],settings.NightS[i]*100,settings.NightV[i]*100);
		if (lastColor.r != temp.r && lastColor.g != temp.g && lastColor.g != temp.g) {
			sameColor = false;
		}
		const button = document.getElementById(id);
	...
	colorChanged(settings.NightH[0],settings.NightS[0]*100,settings.NightV[0]*100,"night");
	...
	// Night start and End
	document.getElementById("nightStart").value = ((settings.StartTime-(settings.StartTime%60))/60<10? "0":"") + (settings.StartTime-(settings.StartTime%60))/60 + ":" + ((settings.StartTime%60<10? "0":"") + settings.StartTime%60);
	document.getElementById("nightEnd").value = ((settings.EndTime-(settings.EndTime%60))/60<10? "0":"") + (settings.EndTime-(settings.EndTime%60))/60 + ":" + ((settings.EndTime%60<10? "0":"") + settings.EndTime%60);
	// Test Night mode
